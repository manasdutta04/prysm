  ...
  try {
    ...
    if (!skipScraping) {
      ...
      if (connectedApps["Play Store"]?.isConnected) {
        ...
        if (!playstoreAppId) {
          const playstoreUser = await PlaystoreUser.findOne({ userId, isConnected: true });
          if (playstoreUser) {
        ...
        if (playstoreAppId) {
          try {
            ...
            for (const review of reviews) {
              const existing = await Feedback.findOne({ userId, externalId: review.id, source: "playstore" });
              if (!existing) {
                await Feedback.create({
                  userId,
                  source: "playstore",
          ...
          try {
            ...
            if (emailList?.messages && emailList.messages.length > 0) {
              for (const msg of emailList.messages) {
                ...
                const existing = await Feedback.findOne({ userId, externalId: msg.id, source: "gmail" });
                if (!existing) {
                  await Feedback.create({
                    userId,
                    source: "gmail",
